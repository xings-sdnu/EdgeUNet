...
class laneDataset(BaseDataSet):
    ...
    def __init__(self, **kwargs):
        ...
        super(laneDataset, self).__init__(**kwargs)

    def _set_files(self):
        self.root = os.path.join(self.root, '')
        self.image_dir = os.path.join(self.root, 'images')
        self.label_dir = os.path.join(self.root, 'gt')
        self.edge_dir = os.path.join(self.root, 'edge_gt')
...
class lane(BaseDataLoader):
    def __init__(self, data_dir, batch_size, split, crop_size=None, base_size=None, scale=True, num_workers=1,
        ...
        if split in ["train_aug", "trainval_aug", "val_aug", "test_aug"]:
            ...
            self.dataset = laneDataset(**kwargs)
        else:
            raise ValueError(f"Invalid split name {split}")
        super(lane, self).__init__(self.dataset, batch_size, shuffle, num_workers, val_split)
