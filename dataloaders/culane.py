...
class CUlaneDataset(BaseDataSet):
    ...
    def __init__(self, **kwargs):
        ...
        super(CUlaneDataset, self).__init__(**kwargs)
...
class CUlane(BaseDataLoader):
    def __init__(self, data_dir, batch_size, split, crop_size=None, base_size=None, scale=True, num_workers=1,
        ...
        if split in ["train_aug", "trainval_aug", "val_aug", "test_aug"]:
            ...
            self.dataset = CUlaneDataset(**kwargs)
        else:
            raise ValueError(f"Invalid split name {split}")
        super(CUlane, self).__init__(self.dataset, batch_size, shuffle, num_workers, val_split)
