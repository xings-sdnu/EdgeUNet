...
class BaseDataSet(Dataset):
    def __init__(self, root, split, mean, std, base_size=None, augment=True, val=False,
                 crop_size=None, scale=True, flip=True, rotate=False, blur=False, return_id=False):
        self.root = root
        self.split = split
        self.mean = mean
        self.std = std
    ...
    def _val_augmentation(self, image, label, edge):
        if self.crop_size:
            h, w = label.shape
            ...
            edge = edge[start_h:end_h, start_w:end_w]
        return image, label, edge

    def _augmentation(self, image, label, edge):
        h, w, _ = image.shape
        ...
        if self.rotate:
            ...
            edge = cv2.warpAffine(edge, rot_matrix, (w, h),
                               flags=cv2.INTER_NEAREST)  # ,  borderMode=cv2.BORDER_REFLECT)
        ...
        if self.crop_size:
            ...
            if pad_h > 0 or pad_w > 0:
                ...
                edge = cv2.copyMakeBorder(edge, value=0, **pad_kwargs)
    ...
    def __getitem__(self, index):
        image, label, image_id, edge = self._load_data(index)
        # image, label, image_id = self._load_data(index)
        if self.val:
            image, label, edge = self._val_augmentation(image, label, edge)
        elif self.augment:
            image, label, edge = self._augmentation(image, label, edge)

        label = torch.from_numpy(np.array(label, dtype=np.int32)).long()
        edge = torch.from_numpy(np.array(label, dtype=np.int32)).long()
        image = Image.fromarray(np.uint8(image))
        if self.return_id:
            return self.normalize(self.to_tensor(image)), label, image_id
        return self.normalize(self.to_tensor(image)), label, edge
