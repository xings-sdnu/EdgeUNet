...
class DataPrefetcher(object):
    ...
    def preload(self):
        try:
            self.next_input, self.next_target, self.next_edge = next(self.loaditer)
        ...
        with torch.cuda.stream(self.stream):
            ...
            self.next_edge = self.next_edge.cuda(device=self.device, non_blocking=True)
